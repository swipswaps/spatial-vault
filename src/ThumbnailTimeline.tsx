...
// Resize an image to a thumbnail size using canvas
function resizeImage(dataUrl: string, maxWidth: number, maxHeight: number): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Calculate new dimensions (cover 16:9 crop)
...
export function ThumbnailTimeline({ onSelectThumbnail }: Props) {
  ...
  const selectFolder = async () => {
    ...
    if (!('showDirectoryPicker' in window)) {
      setError('Your browser does not support the File System Access API. Use Chrome/Edge.');
      return;
    }
    ...
    try {
      ...
      // Correct JavaScript regex – no Python named groups
        ...
        for await (const entry of handle.values()) {
          if (entry.kind === 'file' && entry.name.endsWith('.png')) {
            ...
            if (match) {
              ...
              if (rawDataUrl) {
                // Resize to 200x112 for faster rendering
                const resized = await resizeImage(rawDataUrl, 200, 112);
                entries.push({ filename: entry.name, video_url: videoUrl, timestamp, dataUrl: resized });
              }
            }
          } else if (entry.kind === 'directory') {
            // Recurse into subfolders (optional)
            try {
  ...
  if (loading) {
    return (
      <div style={{ margin: '2rem 0', padding: '1rem', color: '#94a3b8' }}>
        ⏳ Loading thumbnails from {folderName || 'folder'}...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ margin: '2rem 0', padding: '1rem', color: '#f87171', background: '#1e293b', borderRadius: '8px' }}>
        ...
        <br />
        <button
          onClick={() => { setError(null); setItems([]); }}
          style={{ marginTop: '0.5rem', padding: '0.3rem 1rem', background: '#334155', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
        >
          Retry
        </button>
      </div>
  ...
  if (items.length === 0 && !loading) {
    return (
      <div style={{ margin: '2rem 0', padding: '1rem', color: '#94a3b8', textAlign: 'center' }}>
        No thumbnails found in the selected folder.
        <br />
        <button
          onClick={selectFolder}
          style={{ marginTop: '0.5rem', padding: '0.3rem 1rem', background: '#6366f1', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
        >
    ...
    <div style={{ margin: '2rem 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
        <h3 style={{ margin: 0, fontSize: '1.1rem' }}>
          📸 Screenshot Timeline ({items.length}) {folderName && `– ${folderName}`}
        </h3>
        <button
          onClick={selectFolder}
          style={{
